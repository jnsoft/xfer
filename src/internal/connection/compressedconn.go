...
var ErrCompressionMismatch = errors.New("compression setting does not match peer")
...
func NegotiateCompression(conn net.Conn, isServer, enabled bool) error {
    local := byte(0)
    if enabled {
        local = 1
    }
    ...
    if isServer {
        ...
        if _, err := conn.Write([]byte{local}); err != nil {
            return err
        }
    } else {
        if _, err := conn.Write([]byte{local}); err != nil {
